...
namespace PegionManager
{
    public partial class HealthManagerAdd : Form
    {
        ...
        public HealthManagerAdd(Dictionary<string, int> healthManagerPig)
        {
            ...
            {
                this.richTextBox4.AppendText(item.Key + "\r\n");
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string[] pigNumArray = pigNum.Split('\n');
            ...
            {
                string sqlStr = string.Format("INSERT INTO `pigeonmanage`.`pigeondiseasemanage` (`鸽子keyID`, `发病日期`, `疑似疾病`, `确诊疾病`, `基本症状`) VALUES ( '{0}', '{1}', '{2}', '{3}', '{4}')", item.getPigkeyID(), item.发病日期.ToString("yyyy-MM-dd"), item.疑似疾病, item.确诊疾病, item.基本症状);
